...
object MyProgram:
  ...
  def fib(n: Int): Int = {
    def rec(count: Int, prev: Int, cur: Int): Int =
      if (count == 0) then prev
      else rec(count - 1, cur, prev + cur)
    rec(n, 0, 1)
  }
...
object PolymorphicFunctions:
  ...
  def curry[A, B, C](f: (A, B) => C): A => (B => C) = y => (x) => f(y, x)

  // NB: The `Function2` trait has a `curried` method already

  // Exercise 4: Implement `uncurry`
  def uncurry[A, B, C](f: A => B => C): (A, B) => C = (x1, x2) => f(x1)(x2)
